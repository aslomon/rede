...
- **UI**: in-app text is German (informal, concise); code/comments/commits/docs are English. Menu bar popover is 410 pt wide. Reuse existing styles (`PopoverActionButtonStyle`, `SectionLabel`, `RedeStatusPill`, `MenuBarTokens`, …) and preserve mode accent colors. Don't add brand colors/gradients without updating `DESIGN.md`.
