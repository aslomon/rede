...
struct InstallLocationStepView: View {
  @State private var currentInstallLocation = RedeInstallLocationService.currentInstallLocation
    ...
    VStack(alignment: .leading, spacing: OnboardingChrome.contentSpacing) {
      OnboardingCard(accent: isInApplications ? nil : .orange) {
        VStack(alignment: .leading, spacing: 10) {
          ...
          Text(RedeInstallLocationService.bundleURL.path)
          ...
          HStack(spacing: 8) {
            if RedeInstallLocationService.shouldOfferMoveToApplications {
              Button {
                moveToApplications()
              } label: {
            ...
            Button {
              revealInFinder(urls: [RedeInstallLocationService.bundleURL])
            } label: {
        ...
        OnboardingCard(accent: .orange) {
          VStack(alignment: .leading, spacing: 8) {
            ...
            Text("weitere rede-kopien können doppelte login-items auslösen.")
              ...
              .foregroundStyle(.secondary)
            Button {
              revealInFinder(urls: RedeInstallLocationService.otherInstalledBundleURLs)
            } label: {
  ...
  private func moveToApplications() {
    errorText = nil
    do {
      try RedeInstallLocationService.moveToApplicationsAndRelaunch()
    } catch {
      errorText = error.localizedDescription
    }
    currentInstallLocation = RedeInstallLocationService.currentInstallLocation
  }
